...
def db_create_user(username: str, password: str):
    """회원가입: 아이디 중복 확인 후 생성. 성공 시 user dict 반환, 실패 시 None"""
    try:
...
def db_login(username: str, password: str):
    """로그인: 아이디+비밀번호 확인. 성공 시 user dict 반환"""
...
def page_login():
    ...
    with tab_signup:
        new_id = st.text_input("아이디 (영문, 숫자, 4자 이상)", key="signup_id")
...
def page_account():
    ...
    if not st.session_state.confirm_delete_account:
        if st.button("회원탈퇴", type="secondary", use_container_width=True):
            st.session_state.confirm_delete_account = True
            st.rerun()
    else:
        st.error("정말로 탈퇴하시겠습니까? 이 작업은 되돌릴 수 없습니다.")
        c1, c2 = st.columns(2)
        with c1:
            if st.button("✅ 네, 탈퇴합니다", type="primary", use_container_width=True):
                ok = db_delete_user(st.session_state.user_id)
                if ok:
                    st.success("탈퇴가 완료됐습니다.")
                    do_logout()
        with c2:
...
def page_group_room():
    ...
    with c_hdr1:
        if st.button("👥 참여자 목록 확인"):
            st.write(f"현재 참여자 ({len(g_members)}명): {', '.join(list(g_members.keys()))}")
    with c_hdr2:
