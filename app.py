...
with col_m:
    min_h_options = [i * 0.25 for i in range(4, 49)] # 1시간부터 12시간까지 15분 단위
    def format_min_h(x):
        h = int(x)
        m = int((x - h) * 60)
...
time_options.append((24, 0))

with col_t1:
    start_options = time_options[:-1]
...
if st.button("📊 일정 대조하기", type="primary", use_container_width=True):
    if time_start_q >= time_end_q:
        ...
        st.session_state.grp_time_start   = time_start_q
